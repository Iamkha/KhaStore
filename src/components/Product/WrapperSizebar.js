...
const WrapperSizebar = ({ ...props }) => {
  ...
  return (
    <div className="w-full border-b-[1px] pt-[10px]  mx-[10px] pb-[10px]">
      ...
      <div>
        {open && (
          <div className="py-[10px] ease-in-out">
            {arr.map((data) => (
              <div key={data.id}>
                <button className="text-[15px] cursor-pointer mt-[10px] ease-in-out text-slate-600 ">
                  {data.name}
                </button>
              </div>
            ))}
